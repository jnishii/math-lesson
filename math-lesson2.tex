...
%  \footnote{数学の基礎訓練Iの三角関数のセクションで述べたように$x\ll 1$に対して$\sin x\simeq x$となる。}, \;
...
ただし，$f(x)<0$となる領域においては，微小面積$dx f(x)$も負となり ，それを足し合わせた総和も負となる 。
すなわち，\textbf{$f(x)<0$となる領域の面積は負の値}で示す。

\question
定積分は不定積分を利用して計算できる。
\begin{enumerate}
\item ある関数$f(x)$の原始関数の1つを$F(x)$とおくと次式が成立することを証明しなさい。
\begin{align}
  \int_a^bf(x)=F(b)-F(a)\notag
\end{align}
\item 次式を証明しなさい。
...
以下の問に答えなさい
\answer{
(\ref{item:bucket}) 水の量は$\displaystyle V=\int_0^tadt$，
微分方程式は$\dot{V}=a$。ただし$V(0)=0$。
(\ref{item:bucket-hole})
%積分方程式は$\displaystyle V=b-\int_0^tkVdt$ ($k$は正の定数)。微分方程式は
$\displaystyle \dot{V}=-kV$ ($k$は正の定数)。ただし，$V(0)=b$.
(\ref{item:bucket-in-out})
%積分方程式は$\displaystyle V=\int_0^t(a-kV)dt$ ($k$は正の定数)。微分方程式は
$\displaystyle \dot{V}=a-kV$ ($k$は正の定数)。ただし，$V(0)=0$.
